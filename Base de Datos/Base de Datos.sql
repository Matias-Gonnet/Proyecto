...
BEGIN
    ...
    IF nombre_rol IS NULL OR TRIM(nombre_rol) = '' THEN
        return -1;
    ELSE
        SELECT COUNT(*) INTO existe_rol FROM roles WHERE nombre_rol = nombre_rol;
...
BEGIN
    ...
    IF documento IS NULL OR contrasena IS NULL OR correo IS NULL THEN
        return -1;
    
    ELSEIF correo NOT LIKE '%@%.%' THEN
        return -2;
    
    ELSE
        SELECT COUNT(*) INTO existe_rol FROM roles WHERE id_rol = id_rol;
        
        IF existe_rol = 0 THEN
            return -3;
        ELSE

            SELECT COUNT(*) INTO existe_usuario FROM usuarios 
            ...
            ELSE
                ...
                INSERT INTO usuarios (documento, contrasena, correo, id_rol, activo)
                VALUES (documento, contrasena, correo, id_rol, TRUE);
